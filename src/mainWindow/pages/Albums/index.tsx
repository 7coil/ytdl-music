...
> {
  ...
  constructor(props: PropsFromRedux) {
    ...
    this.handleArtistOverrideChange = this.handleArtistOverrideChange.bind(
      this
    );
    this.handleAlbumTitleOverrideChange = this.handleAlbumTitleOverrideChange.bind(
      this
    );
  ...
  handleDownloadButton(): void {
    remote.dialog
      ...
      .then((result) => {
        ...
        if (this.state.overwriteAlbumTitle)
          additionalMetadata.album = this.state.overwriteAlbumTitle;
        if (this.state.overwriteArtist)
          additionalMetadata.artist = this.state.overwriteArtist;
  ...
  handleDownloadMessage(e, text: string, number?: number): void {
    ...
    let statusText = text;
    if (number)
      statusText += ` (${number + 1} of ${
        this.state.selectedAlbum.songs.length
      })`;
    this.setState({
  ...
  render(): ReactElement {
    ...
    try {
      if (window.localStorage.getItem("genres")) {
        extraGenres = JSON.parse(
          window.localStorage.getItem("genres")
        ) as string[];
      }
    } catch (e) {
    ...
    return (
      <PageContainer>
        ...
        {albums.length === 0 && <p>There are no albums to view</p>}
        {albums.length > 0 && (
          <ListView
            listSource={albums.map((album, index) => (
              <p key={index}>
                {album.title}
                <br />
                {album.artist}
            ...
            onChooseItem={(index) => {
              ...
              const isBySingleArtist = newAlbum.songs.every(
                (song) => song.artist === newAlbum.songs[0].artist
              );
        ...
        <ContentDialog
          ...
          onCloseDialog={() => {
            if (this.state.downloadStatus === DOWNLOAD_STATUS.downloaded)
              this.setState({ downloadStatus: DOWNLOAD_STATUS.ready });
          }}
